...
class SDMGRHead(BaseModule):
    ...
    def __init__(
        ...
        edge_embed: int = 256,
        num_gnn: int = 2,
        seq2seq_type: str = "lstm",
        bidirectional: bool = True,
        relation_norm: float = 10.0,
        ...
        self.node_embed = nn.Embedding(
            self.dictionary.num_classes, node_input, self.dictionary.padding_idx
        )
        hidden = node_embed // 2 if bidirectional else node_embed

        seq2seq_supported_types = ["bart", "lstm"]
        assert (
            seq2seq_type in seq2seq_supported_types
    ...
    def predict(
        self, inputs: Tensor, data_samples: List[KIEDataSample]
    ) -> List[KIEDataSample]:
    ...
    def forward(
        ...
        all_edges = torch.cat(
            [relation.view(-1, relation.size(-1)) for relation in relations], dim=0
        )
    ...
    def convert_texts(
        ...
        nodes = (
            torch.zeros((sum(node_nums), max_len), dtype=torch.long)
            + self.dictionary.padding_idx
        )
        for i, text_idx in enumerate(text_idxs):
            nodes[i, : len(text_idx)] = torch.LongTensor(text_idx)
        char_nums = (
            (nodes != self.dictionary.padding_idx).sum(-1).split(node_nums, dim=0)
        )
    ...
    def compute_relations(self, data_samples: List[KIEDataSample]) -> Tensor:
        ...
        for data_sample in data_samples:
            ...
            w, h = torch.clamp(x2 - x1 + 1, min=1), torch.clamp(y2 - y1 + 1, min=1)
...
class GNNLayer(nn.Module):
    ...
    def forward(
        self, nodes: Tensor, edges: Tensor, nums: List[int]
    ) -> Tuple[Tensor, Tensor]:
        """Forward function.
        ...
        for num in nums:
            ...
            residuals.append(
                (residual * cat_nodes[start : start + num**2].view(num, num, -1)).sum(
                    1
                )
            )
            start += num**2
